...
while read -r line; do
    ...
    echo bash ~/Work/pipelines/BLISS/bin/bliss.sh "$code" human ~/Work/pipelines/BLISS/patterns/"$code"_*_"$barcode" 60 "$dir" >> ./runs/run_pipeline_$run.sh
    # echo mkdir -p $datadir/"$code"/outdata_"$barcode" >> ./runs/run_pipeline_$run.sh
    # echo mv $datadir/"$code"/outdata/* $datadir/"$code"/outdata_"$barcode" >> ./runs/run_pipeline_$run.sh
    echo >> ./runs/run_pipeline_$run.sh
done < $inputfile
